...
int ESP32SJA1000Class::parsePacket() // 受信データの解析
{
  if ((readRegister(REG_SR) & 0x01) != 0x01) { // 受信データがない場合
    // no packet
    return 0;
  }

  _rxExtended = (readRegister(REG_SFF) & 0x80) ? true : false; // 受信データの拡張フレームかどうか
  _rxRtr = (readRegister(REG_SFF) & 0x40) ? true : false; // 受信データのリモートフレームかどうか
  _rxDlc = (readRegister(REG_SFF) & 0x0f); // 受信データの長さ
  _rxIndex = 0; // 受信データのインデックス
  ...
  if (_rxRtr) {
    ...
    for (int i = 0; i < _rxLength; i++) { // 受信データの読み込み
      _rxData[i] = readRegister(dataReg + i); 
    }
  }

  // release RX buffer
  modifyRegister(REG_CMR, 0x04, 0x04); // 受信バッファの解放
